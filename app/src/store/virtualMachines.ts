...
export const actionCreators = {
    loadVirtualMachines: (): AppThunkAction<any> => (dispatch) => {
        fetch("https://azureproxy.azurewebsites.us/virtualMachines/allVms", {
            ...
            .then(res => res.json())
            .then(data => {
                dispatch({ type: constants.loadVirtualMachines, virtualMachines: data })
            })
    }
}

export const reducer: Reducer<types.virtualMachines> = (state: types.virtualMachines, incomingAction: Action) => {
