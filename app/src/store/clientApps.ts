...
export const actionCreators = {
    ...
    addClientApp: (appName): AppThunkAction<any> => async (dispatch) => {
        const response = await fetch("https://azureproxy.azurewebsites.us/provision/client?appName=" + appName, {
            method: 'POST',
            headers: new Headers({
                'Authorization': 'Bearer ' + process.env.REACT_APP_AZURE_PROXY,
...
export const reducer: Reducer<types.clientApps> = (state: types.clientApps, incomingAction: Action) => {
    ...
    switch (action.type) {
        ...
        case constants.addClientApp:
            return { ...state, clientApps: state.clientApps.concat(action.clientApp) }
    }
