...
export class Configure extends React.Component<props, state> {
  ...
  allApplications() {
    return this.props.apiApps
      .concat(this.props.clientApps)
      .concat(this.props.serverlessApps);
  }
  ...
  setDeploymentSource() {
    this.setState({ spinner: true }, async () => {
      const newSource = await setDeploymentSource(
        this.state.appName,
        this.allApplications().find(x => x.name == this.state.appName)
          .resourceGroup,
        this.state.deploymentSource,
        this.state.branch
      );
      this.setState({
  ...
  setAppSettings() {
    this.setState({ spinner: true }, async () => {
      const newSettings = await setAppSettings(
        this.state.appName,
        this.allApplications().find(x => x.name == this.state.appName)
          .resourceGroup,
    ...
    return (
      ...
      >
        ...
        <div className="panel panel-body">
          <h2>
            Configure <span style={{ fontSize: ".5em" }}> PaaS services</span>
          </h2>
          <hr />
          <AppSelection
        ...
        {this.state.spinner && (
          <Spinner
            notice={"...loading " + this.state.appName + " configuration..."}
          />
        )}
        {this.state.confirmationModal && (
          <ConfirmationModal setState={this.setState.bind(this)} />
        )}
      </div>
    );
  }
